...
class Pacote extends Record
{
   ...
   public function get_pacoteByPredicado() {
      $criteria = new Criteria;
      $criteria->add(new Filter('id_predicado', '=', $this->id_predicado));
      $repository = new Repository('App\Model\Servico\Pacote');
      // Verficando se encontrou
      return $repository->load($criteria);    
   }

   public function get_pacoteHasPredicado() {
      $criteria = new Criteria;
      $criteria->add(new Filter('id_pacote', '=', $this->id_pacote ?? $this->id));
      $criteria->add(new Filter('id_predicado', '=', $this->id_servico));
      $repository = new Repository('App\Model\Servico\PacotePredicado');
      // Verficando se encontrou
      return $repository->load($criteria);    
   }
