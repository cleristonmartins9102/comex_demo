...
final class Repository
{
    ...
    function load(Criteria $criteria, $dump = false)
    {
        ...
        if ($criteria)
        {
            if (is_null($criteria->getColumn())) {
                $colunm = '*';
            } else {
                $colunm = $criteria->getColumn();
            }
        }
        // instancia a instrução de SELECT
        $sql = "SELECT $colunm FROM " . constant($this->activeRecord.'::TABLENAME');
        ...
        if ($criteria)
        {
            ...
            if (!is_null($criteria->getGroupBy())) {
        ...
        if ($conn = Transaction::get())
        {
            ...
            $teste = utf8_decode('Unitilizacão');
    ...
    function dump(Criteria $criteria) {
