...
@Component({
  selector: 'app-lista-predicado',
  templateUrl: './lista-predicado.component.html',
  styleUrls: ['./lista-predicado.component.css'],
})
export class ListaPredicadoComponent {
  data: any = {
    menu: '',
  ...
  constructor(
    ...
    const menu = new Menu('comercial', 'servico', 'lista de predicados');
  ...
  menuResponse(recordInfo) {
    ...
    switch (recordInfo.event) {
      case 'crud':
      case 'rw':
        this.router.navigate([`/${recordInfo.module}/servico/${recordInfo.appname}/editar`, recordInfo.id]);
        break;

      case 'r':
        this.router.navigate([`/${recordInfo.module}/servico/${recordInfo.appname}/view`, recordInfo.id]);
        break;
