...
export class FormDropdownService {
  ...
  getCaptacao() {
    return this.http.post(`${this.bk_full_address}/captacao/lista/dropdownparam`, JSON.stringify({status: 'atracado'}))
    .pipe(
  ...
  getLiberacaoStatus() {
    return this.http.post(`${this.bk_full_address}/liberacaostatus/lista/all`, '')
    .pipe(take(1));
  }
}
